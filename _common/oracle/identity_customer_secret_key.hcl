...
locals {
  ...
  module_dir          = "oracle/identity_customer_secret_key"
  module_version      = "main"
...
inputs = {
  ...
  name             = "admin"
  user_id          = run_cmd("--terragrunt-quiet", "${get_repo_root()}/.kek.sh", "user", "${get_env("OCI_CONFIG_PATH")}", "${local.oracle_profile_name}")
}
